<!DOCTYPE html>
<html lang="en-US">

	<?php
		$page_title = 'Team';
		include('includes/head.php');
	?>

	<body class="team">
		<div class="body-overflow">

			<?php include('includes/fs-menu.php'); ?>
			<?php include('includes/top-menu.php'); ?>

			<main id="main-content">

				<!-- banner -->
				<section id="banner">
					<div class="container container-big">
						<div class="row">

							<div class="col-md-5 left">

								<h2 class="text-bigger title wow fadeInUp">
									Team
								</h2>

							</div>

							<div class="col-md-7 right">

								<p class="text-bigger wow fadeIn sub">
									As a boutique political consultancy, our team values customization and proximity to meet client demands.
								</p>

								<p class="text-medium wow fadeIn desc">
									To understand complex political and institutional scenarios, we value different areas of activity. We have consultants with a history of working in the Executive, Legislative, Judiciary, municipalities, financial sector and journalism.
								</p>

							</div>

						</div>
					</div>
				</section>

				<!-- grid -->
				<section id="grid">
					
					<div class="link wow fadeIn" data-wow-delay=".1s">
						<a href="rafael-favetti">
							
							<img src="../assets/img/team/rafael.jpg" alt="Rafael Favetti">
							
							<h3 class="text-bigger">
								Rafael <br />
								Favetti
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="logo wow fadeIn" data-wow-delay=".2s">
						<?php include('../assets/svg/letter-f.php') ?>
					</div>

					<div class="link wow fadeIn" data-wow-delay=".3s">
						<a href="guilherme-favetti">
							
							<img src="../assets/img/team/guilherme.jpg" alt="Guilherme Favetti">
							
							<h3 class="text-bigger">
								Guilherme <br />
								Favetti
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="photo wow fadeIn" data-wow-delay=".4s">
						<img src="../assets/img/grid-02.jpg" alt="">
					</div>

					<div class="photo wow fadeIn" data-wow-delay=".1s">
						<img src="../assets/img/grid-01.jpg" alt="">
					</div>

					<div class="link wow fadeIn" data-wow-delay=".2s">
						<a href="bernardo-livramento">
							
							<img src="../assets/img/team/bernardo.jpg" alt="Bernardo Livramento">
							
							<h3 class="text-bigger">
								Bernardo <br />
								Livramento
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="link wow fadeIn" data-wow-delay=".3s">
						<a href="bruno-rizzi">
							
							<img src="../assets/img/team/bruno.jpg" alt="Bruno Rizzi">
							
							<h3 class="text-bigger">
								Bruno <br />
								Rizzi
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="link wow fadeIn" data-wow-delay=".4s">
						<a href="mariana-londres">
							
							<img src="../assets/img/team/mariana.jpg" alt="Mariana Londres">
							
							<h3 class="text-bigger">
								Mariana <br />
								Londres
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="text wow fadeIn" data-wow-delay=".1s">
						<p class="text-medium">
							Intelligence to anticipate political and regulatory risks.
						</p>
					</div>

					<div class="photo wow fadeIn" data-wow-delay=".2s">
						<img src="../assets/img/grid-03.jpg" alt="">
					</div>

					<div class="text wow fadeIn" data-wow-delay=".3s">
						<p class="text-medium">
							We are a multidisciplinary team.
						</p>
					</div>

					<div class="link wow fadeIn" data-wow-delay=".4s">
						<a href="giovanna-favetti">
							
							<img src="../assets/img/team/giovanna.jpg" alt="Giovanna Favetti">
							
							<h3 class="text-bigger">
								Giovanna <br />
								Favetti
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="photo wow fadeIn" data-wow-delay=".1s">
						<img src="../assets/img/grid-04.jpg" alt="">
					</div>

					<div class="link wow fadeIn" data-wow-delay=".2s">
						<a href="arthur-lira">
							
							<img src="../assets/img/team/arthur.jpg" alt="Arthur Lira">
							
							<h3 class="text-bigger">
								Arthur <br />
								Lira
							</h3>

							<div class="more">
								<span></span>
								<span></span>
							</div>

						</a>
					</div>

					<div class="photo wow fadeIn" data-wow-delay=".3s">
						<img src="../assets/img/grid-05.jpg" alt="">
					</div>

					<div class="link wow fadeIn" data-wow-delay=".4s">
						<img src="../assets/img/grid-06.jpg" alt="">
					</div>

				</section>

				<!-- affiliations -->
				<section id="affiliations">
					<div class="container container-big">
						<div class="row">
							<div class="col-12">

								<h2 class="text-bigger feature wow fadeInUp">
									Affiliations
								</h2>

								<div class="blocks">
									<div class="row">

										<div class="col-sm-6 wow fadeInUp">

											<p class="text-small">
												01.
											</p>

											<a href="https://www.favetti.adv.br/" target="_blank" class="text-medium hover-multiple-lines">
												Favetti Lawyer Society
											</a>

										</div>

										<div class="col-sm-6 wow fadeInUp" data-wow-delay=".1s">

											<p class="text-small">
												02.
											</p>

											<a href="https://abrig.org.br/" target="_blank" class="text-medium hover-multiple-lines">
												Brazilian Association of Institutional and Governmental Relations – (ABRIG)
											</a>

										</div>

										<div class="col-sm-6 wow fadeInUp">

											<p class="text-small">
												03.
											</p>

											<a href="https://ivnl.com.br/" target="_blank" class="text-medium hover-multiple-lines">
												Institute Victor Nunes Leal
											</a>

										</div>

										<div class="col-sm-6 wow fadeInUp" data-wow-delay=".1s">

											<p class="text-small">
												04.
											</p>

											<a href="https://www.migalhas.com.br/coluna/inteligencia-politica" target="_blank" class="text-medium hover-multiple-lines">
												Migalhas
											</a>

										</div>

									</div>
								</div>

							</div>
						</div>
					</div>
				</section>
				
			</main>

			<!-- footer -->
			<?php include('includes/footer.php'); ?>

			<footer>
				<script src="../assets/js/framework.js"></script>
				<script src="../assets/js/team.js" defer></script>
			</footer>

		</div>
	</body>
</html>