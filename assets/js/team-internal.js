if (window.location.href.indexOf("rafael-favetti") > -1) {
	$('#related .rafael').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}

if (window.location.href.indexOf("guilherme-favetti") > -1) {
	$('#related .guilherme').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}

if (window.location.href.indexOf("bernardo-livramento") > -1) {
	$('#related .bernardo').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}

if (window.location.href.indexOf("israel-medeiros") > -1) {
	$('#related .israel').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}

if (window.location.href.indexOf("arthur-lira") > -1) {
	$('#related .arthur').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}

if (window.location.href.indexOf("giovanna-favetti") > -1) {
	$('#related .giovanna').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}

if (window.location.href.indexOf("anna-dantas") > -1) {
	$('#related .anna').remove();
	$('#top-menu .top-menu li:nth-child(3) a').addClass('active');
	$('#fs-menu .fs-menu li:nth-child(3) a').addClass('active');
}